...
namespace Coffee.AsmdefEx
{
    ...
    {
        const string DllPublishPathPref = "OpenSesameDllPublishPath";
        ...
        static void OnAssemblyCompilationFinished(string name, CompilerMessage[] messages)
        {
            try
            {
                ...
                var path = EditorPrefs.GetString(DllPublishPathPref);
                //var path = Path.GetDirectoryName(Path.GetDirectoryName(originPath));

                // Publish a dll to parent directory.
                var dst = Path.Combine(path, assemblyName + ".dll");
        ...
        static void OnPostHeaderGUI(Editor editor)
        {
            ...
            if (s_OpenSettings)
            {
                ...
                using (new GUILayout.VerticalScope(EditorStyles.helpBox))
                {
                    ...
                    var publishPath = EditorPrefs.GetString(DllPublishPathPref);
                    EditorGUI.BeginChangeCheck();
                    publishPath = EditorGUILayout.TextField("Publish Path", publishPath);
                    if (EditorGUI.EndChangeCheck())
                        EditorPrefs.SetString(DllPublishPathPref, publishPath);

                    // Publish assembly as a dll.
                    using (new EditorGUI.DisabledScope(string.IsNullOrWhiteSpace(publishPath))) 
                    {
